...
<body>
	<script>
		...
		$(document).ready(function() {
			// EJERCICIO 2: Manejador de evento para copiar automáticamente 
            // el email como nick del usuario
			...
			// EJERCICIO 4: Uso de AJAX con JQuery para
            // cargar de manera asíncrona los municipios 
            // según la provincia seleccionada
		...
		<fieldset><legend>Datos personales</legend>
			<div> <label for="nif">NIF<em>*</em></label>
			    <input id="nif" name="nif" type="text" 
						...
						value="<?php echo $formulario['nif'];?>" required>
			</div>

			<div> <label for="nombre">Nombre:<em>*</em></label>
			     <input id="nombre" name="nombre" type="text" size="40" 
				        value="<?php echo $formulario['nombre'];?>" required/>
			</div>

			<div> <label for="apellidos">Apellidos:</label>
			      <input id="apellidos" name="apellidos" type="text" size="80" 
			...
			<div> <label>Perfil:</label>
			     <label>
				        <input name="perfil" type="radio" value="ALUMNO" 
						       <?php if($formulario['perfil']=='ALUMNO') echo ' checked ';?>/>
				         Alumno
				 </label>
			     <label>
				         <input name="perfil" type="radio" value="PDI" 
						        <?php if($formulario['perfil']=='PDI') echo ' checked ';?>/>
				         PDI
				 </label>
			     <label>
				        <input name="perfil" type="radio" value="PAS" 
						       <?php if($formulario['perfil']=='PAS') echo ' checked ';?>/>
				        PAS
				 </label>
			</div>
			...
			<div> <label for="email">Email:<em>*</em></label>
			     <input id="email" name="email"  type="email" p
				  laceholder="usuario@dominio.extension" 
		...
		<fieldset><legend>Datos de cuenta</legend>
			...
			<div><label for="pass">Password:<em>*</em></label>
                <input type="password" name="pass" id="pass" 
                       placeholder="Mínimo 8 caracteres entre letras y dígitos" required oninput="passwordValidation(); "/>
			</div>
					...
					<?php
						$generos = listarGeneros($conexion);
				  		foreach($generos as $genero) {
							  // Chequea si este género literario fue seleccionado previamente, 
							  // para marcarlo en el HTML con el atributo "selected"
		...
		<fieldset><legend> Dirección </legend>
			...
			<div> <label for="calle">Calle/Avda.:<em>*</em></label>
			      <input id="calle" name="calle" type="text" size="80" 
				         value="<?php echo $formulario['calle'];?>" required/>
			</div>

			<div> <label for="provincia">Provincia:<em>*</em></label>
			       <input list="opcionesProvincias" name="provincia" id="provincia" required 
				          value="<?php echo $formulario['provincia'];?>"/>
			...
			<div><label for="municipio">Municipio:<em>*</em></label>
			     <input id="municipio" name="municipio" type="text" 
				        list="opcionesMunicipios" required 
						value="<?php echo $formulario['municipio'];?>">
